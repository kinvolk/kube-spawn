...
func checkK8sStableRelease(k8srel string) bool {
	...
	if err != nil {
		stderr.Printf("cannot get constraint for >= %s: %v", k8sStableVersion, err)
		return false
	}
...
func doCheckK8sStableRelease(k8srel string) {
	if !checkK8sStableRelease(k8srelease) {
		stdout.Printf("WARNING: K8s with version <%s is not compatible with kube-spawn.",
			k8sStableVersion)
		stdout.Printf("It's highly recommended to upgrade K8s to 1.7 or newer.")
...
func doSetup(numNodes int, baseImage, kubeSpawnDir string) {
	if err := bootstrap.EnsureBridge(); err != nil {
		stderr.Fatalf("Error checking CNI bridge: %s", err)
	}

	if err := bootstrap.WriteNetConf(); err != nil {
		stderr.Fatalf("Error writing CNI configuration: %s", err)
	}

	stdout.Printf("Checking base image")
	if baseImage == "" {
		stderr.Fatal("No base image specified.")
	}
	if !bootstrap.MachineImageExists(baseImage) {
		stderr.Fatal("Base image not found.")
	}
	...
	if !utils.IsK8sDev(k8srelease) {
		if err := bootstrap.DownloadK8sBins(k8srelease, path.Join(kubeSpawnDir, "k8s")); err != nil {
			stderr.Fatalf("Error downloading k8s files: %s", err)
		}
	}
	...
	if err := os.MkdirAll(ksExtraDir, os.FileMode(0755)); err != nil {
		stderr.Fatalf("Unable to create directory %q: %v.", ksExtraDir, err)
	}
	if err := bootstrap.DownloadSocatBin(ksExtraDir); err != nil {
		stderr.Fatalf("Error downloading socat files: %s", err)
	}
	...
	if poolSize, err = bootstrap.GetPoolSize(baseImage, numNodes); err != nil {
		...
		stderr.Fatalf("cannot get pool size: %v", err)
	}
	...
	for i := 0; i < numNodes; i++ {
		...
		if !bootstrap.MachineImageExists(node.Name) {
			if err := bootstrap.NewNode(baseImage, node.Name); err != nil {
				stderr.Fatalf("Error cloning base image: %s", err)
			}
		}
	...
	if len(nodesToRun) == 0 {
		stdout.Printf("No nodes to create. stop")
		os.Exit(1)
	}

	if err := bootstrap.EnlargeStoragePool(poolSize); err != nil {
		stdout.Printf("Warning: cannot enlarge storage pool: %v", err)
	}

	for _, node := range nodesToRun {
		if err := node.Run(kubeSpawnDir, rktBinDir, rktletBinDir); err != nil {
			stderr.Fatalf("Error running node: %v", err)
		}

		if err := node.Bootstrap(); err != nil {
			stderr.Fatalf("Error running bootstrap script: %v", err)
		}

		stdout.Printf("Success! %s started.", node.Name)
	}

	stdout.Print("All nodes are running. Use machinectl to login/stop/etc.")
	stdout.Printf("KUBECONFIG is in %v", kubeSpawnDir)
}
...
func writeKubeadmBootstrapScript() {
	...
	if outbuf == nil {
		stderr.Fatalf("Error generating kubeadm bootstrap script")
	}

	scriptsDir := filepath.Join(kubeSpawnDir, "scripts")
	if err := os.MkdirAll(scriptsDir, os.FileMode(0755)); err != nil {
		stderr.Fatalf("Unable to create directory %q: %v.", kubeSpawnDir, err)
	}
	...
	if err := ioutil.WriteFile(bootstrapScript, outbuf.Bytes(), os.FileMode(0755)); err != nil {
		stderr.Fatalf("Error writing script file %s: %v", bootstrapScript, err)
	}
}
...
func writeKubeadmInitScript() {
	...
	if outbuf == nil {
		stderr.Fatalf("Error generating kubeadm init script")
	}
	...
	if err := ioutil.WriteFile(initScript, outbuf.Bytes(), os.FileMode(0755)); err != nil {
		stderr.Fatalf("Error writing script file %s: %v", initScript, err)
	}
}
...
func writeKubeadmExtraArgs() {
	...
	default:
		stderr.Fatalf("runtime %s is not supported", k8sruntime)
	}
...
func writeKubeadmConfig() {
	...
	if err := ioutil.WriteFile(kubeadmConfig, outbuf.Bytes(), os.FileMode(0644)); err != nil {
		stderr.Fatalf("Error writing config %s: %v", kubeadmConfig, err)
	}
}
...
func copyConfigFiles() {
	...
	if _, err := utils.CopyFileToDir(filepath.Join(etcSrc, fName), etcDst); err != nil {
		stderr.Fatalf("Error copying file %s: %v", fName, err)
	}
