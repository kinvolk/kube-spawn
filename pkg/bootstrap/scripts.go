...
func GenerateScripts(cfg *config.ClusterConfiguration) error {
    ...
    DockerHttpProxyConfig := fmt.Sprintf("[Service]\nEnvironment=\"HTTP_PROXY=%s\" \"HTTPS_PROXY=%s\" \"NO_PROXY=%s\" \"http_proxy=%s\" \"https_proxy=%s\" \"no_proxy=%s\"", os.Getenv("http_proxy"), os.Getenv("https_proxy"), os.Getenv("no_proxy"), os.Getenv("http_proxy"), os.Getenv("https_proxy"), os.Getenv("no_proxy"))

	if err := fs.CreateFileFromBytes(path.Join(rootfsPath(cfg), script.DockerHttpProxyConfigPath), []byte(DockerHttpProxyConfig)); err != nil {
		return err
	}
